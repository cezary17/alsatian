...
def linear_proxy_quantized(train_features, train_labels, test_features, test_labels, num_classes, device):
    first_item = train_features[0]
    input_dimension = first_item.shape
    ...
    model = torch.ao.quantization.quantize_dynamic(model=model, dtype=torch.qint8)
